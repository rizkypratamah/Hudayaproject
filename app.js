    ...
    //This view returns (i.e. emits) the id, revision number and new_city_name variable of all documents in the DB
  	db.insert(
	 {
		    ...
		    views: {
	  				  "names_database":
	  				   {
	      					"map": "function (doc) {\n  emit(doc._id, [doc._rev, doc.new_name, doc.new_jumlah, doc.new_beban, doc.new_device]);\n}"
	    			   }
      	   		   }
     },
	 function(err, data) {
...
app.get('/view_names',function(req, res){
	...
	request({
		...
		if (!error && response.statusCode === 200)
		{
			var user_data = body.rows;  //body.rows contains an array of IDs, Revision numbers and Names from the view
			//for(var i=0; i<user_data.length; i++)
			//var name_JSON = '{\"new_name\":\"' + user_data[i].value[1] + ',\"new_jumlah\":\"' + user_data[i].value[2] + ',\"new_beban\":\"' + user_data[i].value[3] +',\"new_device\":\"' + user_data[4].new_device +'\"}'; //create an array of names only
	      			
			res.contentType('application/json');
			console.log("Returning names");
			var obj = JSON.stringify(user_data);
			res.send(JSON.parse(obj)); //return the list to front end for display
		}
		else
		{
			console.log("No data from URL");
...
app.get('/add_name',function(req, res){ //to add a city into the database
	...
	req.query.new_jumlah = req.query.new_jumlah;
	req.query.new_beban = req.query.new_beban;
	req.query.new_device = req.query.new_device;
