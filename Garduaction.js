			...
			success: function(data) {
				...
				if(!data.hasOwnProperty("added"))
				{
					...
					document.getElementById("empty_database").style.display="none";
          			...
          			names_table += "<tbody>";
          			names_table += "<tr> <td> Gardu </td> <td> Jumlah Pelanggan </td> <td> Total Beban </td> <td> Device </td>";
          			for(var i = 0; i < data.length; i++)
            			names_table += "<tr class=\"info\"><td colspan=\"1\">" + data[i].value[1] + "</td><td colspan=\"1\">" + data[i].value[2] + "</td><td colspan=\"1\">" + data[i].value[3] + "</td><td colspan=\"1\">" + data[i].value[4] "</td></tr>";
...
*/
	...
	$("#add_name_button").click(function(e){
			...
			$.ajax({
				...
				type: "GET",
				dataType: "json",
				data:{new_name: $("#newname").val(),new_jumlah: $("#newjumlah").val(),new_beban: $("#newbeban").val(),new_device: $("#newdevice").val()},
				contentType: "application/json",
				cache: true,
				timeout: 5000,
				complete: function() {
				...
				success: function(data) {
					...
					else if(data.added === "DB read error")
					{
		   			...
		   			$("#newname").val("");
		    		$("#newname").attr("placeholder", "Enter Gardu");
		    		$("#newjumlah").val("");
		    		$("#newjumlah").attr("placeholder", "Jumlah Pelanggan Baru");
		    		$("#newbeban").val("");
		    		$("#newbeban").attr("placeholder", "Total Beban Baru");
		    		$("#newdevice").val("");
		    		$("#newdevice").attr("placeholder", "Device Baru");
				},
				error: function() {
				  console.log('process error');
				},
	...
	$("#update_name_button").click(function(e){
		...
		$.ajax({
				...
				type: "GET",
				dataType: "json",
				data: {name_list:$("#name_list option:selected").text(), updated_new_jumlah: $("#updated_new_jumlah").val(), updated_new_beban: $("#updated_new_beban").val(),updated_new_device: $("#updated_new_device").val()},
				contentType: "application/json",
				cache: true,
				timeout: 5000,
				complete: function() {
				...
				success: function(data) {
					...
					$("#updated_new_jumlah").val("");
					$("#updated_new_jumlah").attr("placeholder", "Enter Jumlah");
					$("#updated_new_beban").val("");
					$("#updated_new_beban").attr("placeholder", "Enter Beban");
					$("#updated_new_device").val("");
					$("#updated_new_device").attr("placeholder", "Enter Device");
